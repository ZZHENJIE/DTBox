...
- 同步 `docs/web.md` 与 `web/README.md` 增量：`calendar` 改为 Finviz/Benzinga 双源 Tabs（默认 Finviz），Benzinga economics 切 `timestamp` UTC，新增强制财经日历数据源说明节（`ipo`/`spac`/`economics`/`earnings` 端点与表头、类型分离、`finvizEconomics`/`finvizEarnings`、Ticker→Symbol 统一）
